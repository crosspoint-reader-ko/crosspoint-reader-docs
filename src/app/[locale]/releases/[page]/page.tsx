...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "releases.metadata" });
  return {
    title: t("title"),
...
export default async function ReleasesPage({
  ...
  const cardLabels = {
    latest: t("card.latest"),
    preRelease: t("card.preRelease"),
              ...
              {totalPages > 1 && (
                <div className="mt-4 text-center text-sm text-gray-500">
                  {t("pagination.rangeSummary", {
                    ...
                    start: startIndex + 1,
                    end: Math.min(endIndex, releases.length),
                  })}
                </div>
              )}
            </>
